...
class RobotRunner(mesa.Agent): 
    ...
    # Assigns a picker whose basket is more than half full.
    def assign_picker_w2( self ):
    ...
    def assign_picker( self ):
        ...
        self.assign_picker_w2( )
    
    def go_to_packing_station( self ): 

        # If you're at the packing station, deliver fruit.
        if self.at_packing_station( ): 
            # print('Fruit in basket:', self.fruit_in_basket, ' Total fruit delivered: ', self.total_fruit_delivered)
    ...
    def decide_next_move_simple_2 ( self ) : 
    ...
    def decide_next_move_simple_1 ( self ) : 
        ...
        #if the assigned picker is moving to the packing station, drop the picker.
        if self.target_picker!=None: 
            if self.target_picker.status in [Status.DROPPINGOFF, Status.GOING_BACK]:
                self.target_picker.robot_assigned = False
                self.target_picker = None
                self.moves_assigned = [ ] 
