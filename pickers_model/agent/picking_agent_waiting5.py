...
from pickers_model.agent.patience import PatienceTime, PatienceTimeRandom

# Implements pickers that is waiting, but if the robot does not arrive by some time, it goes to the packing station.

# Waiting3 is a picking agent that ... 

class PWaiting5( PickingAgent ):

    def __init__(self, unique_id, model, picker_id = "", patience = 0.0, max_waiting_time = 180.0 ):
        super().__init__( unique_id, model, picker_id, picker_type = PickerType.WAITING5  )
        ...
        self.patience_model = PatienceTimeRandom( patience, max_waiting_time )
        
        # print('Picker ', self.unique_id, 'Wait until: ', self.patience_model.wait_until)
    ...
    def w3_waiting_too_long( self, model ): 

        return self.patience_model.waited_too_long() 
    ...
    def w4_step( self ): 
        ...
        # if self.patience_model.waited_so_far>0:
            # print('Picker:',self.unique_id, self.patience_model.waited_so_far, self.patience_model.waited_too_long())

        if not self.fruit_basket_full( ):
            self.patience_model.reset_waiting( )        
        if len( self.moves_assigned )>0: 
        ...
        if self.at_packing_station( ): 
            # print('Total fruit delivered: ', self.total_fruit_delivered, ' Fruit in basket: ', self.fruit_in_basket )            
            self.total_fruit_delivered += self.fruit_in_basket
            self.fruit_in_basket = 0 
            self.moves_assigned = [  ] 
            # print('Setting status to GOING_BACK. Total fruit delivered: ', self.total_fruit_delivered, ' Fruit in basket: ', self.fruit_in_basket )
            self.status = Status.GOING_BACK
        ...
        if self.fruit_basket_full(  ): 
            ...
            if self.w3_waiting_too_long( self.model ):
                self.patience_model.reset_waiting( )
                #print('Setting status to DROPPINGOFF.')
                self.status = Status.DROPPINGOFF 
                self.go_to_packing_station( )
                #print(self.patience_model.waited_so_far)
            else:
                #print('Setting status to WAITING. Line 192.')
                self.status = Status.WAITING
                self.patience_model.add_time( self.model.step_size )
                return self.pos 
        ...
        # 3. Assign a row. 
            ...
            if self.robot_assigned: 
                # print('Setting status to WAITING. Line 210.')
                self.status = Status.WAITING
                return self.pos 
