...
# Picking agent that waits for the robot runner forever.

class PWaiting1( PickingAgent ):

    def __init__(self, unique_id, model, picker_id = "" ):
        super().__init__( unique_id, model, picker_id, picker_type = PickerType.WAITING1  )
                        
        # list_of_speeds = [ 91.6892502258356, 45.31722054380665, 72.61345852895148, 54.33646812957158, 78.13070463672874, 62.18655967903711, 67.49049429657795, 77.43752199929602, 63.35174953959484 ]
        
        list_of_speeds = [ 32.31292517006803,
                          45.31051964512041,
                          43.84268214055448,
                          58.265582655826556,
                          54.91024287222809,
        ...
        mu = np.mean( list_of_speeds )
        sigma = np.std( list_of_speeds ) 
        
        # picking_speed_kgph = 200
        # a.picking_speed = picking_speed_kgph * 5 / 18  
        self.picking_speed = abs( random.gauss( mu, sigma ) )
