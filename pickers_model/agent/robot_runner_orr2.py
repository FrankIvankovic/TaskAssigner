...
class RobotRunnerOrderedRR2(RobotRunner): 
    ...
    def assign_picker_orr2( self ):
        
        subset_of_pickers = [] 
        for picker in self.model.pickers:
            if not picker.robot_assigned and picker.fruit_in_basket > picker.fruit_basket_capacity/2.0: 
                t = self.model.field_map.topological_map.find_time_to_reach( picker.current_node, picker.model.field_map.find_nearest_packing_station( picker.current_node ), picker.max_speed )
                subset_of_pickers.append( { 'picker': picker, 'time_to_packing_station': t } )

        subset_of_pickers.sort( key=lambda x: x[ 'time_to_packing_station' ], reverse=True )

        #print('List of pickers:')

        #for p in subset_of_pickers:
            #print( p[ 'picker' ].unique_id, p[ 'time_to_packing_station' ] )
